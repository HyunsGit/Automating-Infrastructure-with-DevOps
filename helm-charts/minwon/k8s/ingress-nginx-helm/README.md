# ingress-nginx
중요!!!
values-minwon.yaml은 deprecated니 참고 x(원본 보존용)
values-minwon-v1-1.yaml 확인(현재 사용중인 values manifest)
  - ln.47-50 Ingress Controller DaemonSet configMap 설정
  - ln.117,130 Ingress Controller IngressClass 지정
  - ln.178,179 Ingress Controller Default SSL Certificate 지정
  - ln.193 Ingress Controller는 DaemonSet으로 지정
  - ln.219-223 Ingress Controller DaemonSet toleration 설정
  - ln.301-302 Ingress Controller DaemonSet nodeSelector 설정
  - ln.459-461 Ingress Controller Service annotation 설정
  - ln.479 Ingress Controller Service externalTrafficPolicy 'Local'로 지정
  - ln.506 Ingress Controller Service의 type은 'LoadBalancer'로 지정
  - ln.669 Ingress Controller webhook certgen nodeSelector 설정
  - ln.671-674 Ingress Controller webhook certgen toleration 설정
